...
#pragma warning restore 0649
		...
		private bool drawObjectsToSettingsSection = true;
		...
		private void OnGUI()
		{
			...
			else if( currentPhase == Phase.Setup )
			{
				...
				drawObjectsToSettingsSection = EditorGUILayout.Foldout(drawObjectsToSettingsSection, "Settings");

				if (drawObjectsToSettingsSection)
				{
					Utilities.DrawHeader( "<b>SEARCH IN</b>" );
					
					searchInAssetsFolder =
						WordWrappingToggleLeft("Project window (Assets folder)", searchInAssetsFolder);
					...
					dontSearchInSourceAssets = WordWrappingToggleLeft(
						"Don't search \"SEARCHED OBJECTS\" themselves for references", dontSearchInSourceAssets);
					...
					if (!EditorApplication.isPlaying)
					{
						searchInScenesInBuild =
							WordWrappingToggleLeft("Scenes in Build Settings", searchInScenesInBuild);
					...
					hideDuplicateRows =
						WordWrappingToggleLeft("Hide duplicate rows in search results", hideDuplicateRows);
					hideRedundantPrefabReferencesInAssets = WordWrappingToggleLeft(
						hideRedundantPrefabReferencesInAssetsLabel, hideRedundantPrefabReferencesInAssets);
					hideRedundantPrefabReferencesInScenes = WordWrappingToggleLeft(
						hideRedundantPrefabReferencesInScenesLabel, hideRedundantPrefabReferencesInScenes);
