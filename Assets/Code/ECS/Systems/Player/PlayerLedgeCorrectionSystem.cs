    ...
    public sealed class PlayerLedgeCorrectionSystem : ISystem
    {
        ...
        public void OnUpdate(float deltaTime)
        {
            foreach (var entity in _filter)
            {
                ...
                if (!(slideComponent.IsSliding && isGrounded))
                {
                    characterControllerComponent.CharacterController.stepOffset = 0.28f;
                }

                Vector3 dif = pp - characterControllerComponent.CharacterController.transform.position;
                if (slideComponent.SlopeAngle > 60 && slideComponent.SlopeAngle < 85 && dif.y < 0.05f && dif.y >= 0f)
                {
                    characterControllerComponent.CharacterController.stepOffset = 0.5f;
                }
                else
                {
                    characterControllerComponent.CharacterController.stepOffset = 0.28f;
